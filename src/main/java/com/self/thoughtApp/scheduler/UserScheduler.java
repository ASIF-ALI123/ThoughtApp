package com.self.thoughtApp.scheduler;
...
public class UserScheduler {
    ...
    public void fetchUsersAndSendSaMail() {
        ...
        for (User user : users) {
            List<thoughtEntry> thoughtEntries = user.getThoughtEntries();
            List<Sentiment> sentiments = thoughtEntries.stream().filter(x -> x.getDate().isAfter(LocalDateTime.now().minus(7, ChronoUnit.DAYS))).map(x -> x.getSentiment()).collect(Collectors.toList());
